...
import sun.jvm.hotspot.runtime.riscv32.*;

/** <P> Should be able to be used on all riscv32 platforms we support
    (Linux/riscv32) to implement JavaThread's "currentFrameGuess()"
    functionality. Input is an RISCV32ThreadContext; output is SP, FP,
    and PC for an RISCV32Frame. Instantiation of the RISCV32Frame is
    left to the caller, since we may need to subclass RISCV32Frame to
...
public class RISCV32CurrentFrameGuess {
  private RISCV32ThreadContext context;
  private JavaThread       thread;
  private Address          spFound;
  private Address          fpFound;
  private Address          pcFound;

  private static final boolean DEBUG = System.getProperty("sun.jvm.hotspot.runtime.riscv32.RISCV32Frame.DEBUG")
                                       != null;

  public RISCV32CurrentFrameGuess(RISCV32ThreadContext context,
                              JavaThread thread) {
  ...
  public boolean run(long regionInBytesToSearch) {
    Address sp  = context.getRegisterAsAddress(RISCV32ThreadContext.SP);
    Address pc  = context.getRegisterAsAddress(RISCV32ThreadContext.PC);
    Address fp  = context.getRegisterAsAddress(RISCV32ThreadContext.FP);
    if (sp == null) {
      ...
      if (vm.isClientCompiler()) {
        ...
        for (long offset = 0;
          ...
          try {
            ...
            Frame frame = new RISCV32Frame(curSP, null, pc);
  ...
  /** May be null if getting values from thread-local storage; take
      care to call the correct RISCV32Frame constructor to recover this if
      necessary */
